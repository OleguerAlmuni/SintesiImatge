...
bool Reader::ReadSceneDescription( const char *file_name, Scene &scene, Camera &camera )
{
	...
	for(;;)
	{
		...
		if( Get( line, "diffuse"     , obj->material.diffuse		) ) continue;
		if( Get( line, "specular"    , obj->material.specular		) ) continue;
		if( Get( line, "reflectivity", obj->material.reflectivity	) ) continue;
		if( Get( line, "refractive_index", obj->material.refractive_index	) ) continue;
		if( Get( line, "opacity"		 , obj->material.opacity			) ) continue;
		if( Get( line, "Phong_exp"   , obj->material.phong_exp	) ) continue;
        if( Get( line, "emission"    , obj->material.emission     ) ) continue;
		...
		if( Get( line, "lightpos"  , scene.light[scene.num_lights].position ) )continue;
		...
		if( Get( line, "lightcolor"    , scene.light[scene.num_lights].color  ) ){
			scene.num_lights++;
			continue;            
		}
